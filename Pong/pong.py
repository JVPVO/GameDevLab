...
class Fundo:
    def __init__(self):
        self.sprite = GameImage(image_file='Pong/fundo.png')
        self.drawn = False
    def desenhar(self):
        if not self.drawn:
...
class Raquete:
    def __init__(self, side:int, player:bool):
        self.sprite = Sprite("Pong/raquete.png")
        self.side = side
        self.drawn = False
        self.v1 = 500
        self.player = player
    ...
    def desenhar(self):
        ...
        self.sprite.draw()
        self.drawn = True
    def movement(self, side, objetos):
        x,y = self.sprite.image.get_size()
        if side == -1:
            if self.player == True:
                ...
                for objeto in objetos:
                    if self.sprite.y > objeto.sprite.y:
                        raquete.sprite.y -= self.v1 * janela.delta_time()
                    elif self.sprite.y + y < objeto.sprite.y:
                        raquete.sprite.y += self.v1 * janela.delta_time()
        elif side == 1:
...
raquetes = [Raquete(1, player=True), Raquete(-1, player=False)]
fundo = Fundo()
...
while True:
    ...
    fundo.desenhar()
    janela.draw_text(f'{pontos[1]} x {pontos[0]}',janela.width/2-40, janela.height - 0.9*janela.windowed_height,size=36)
    for raquete in raquetes:
        raquete.desenhar()
        raquete.movement(raquete.side, objetos)
    for objeto in objetos:
        objeto.desenhar()
        pos = objeto.colisao(objeto.sprite.x, objeto.sprite.y)
        objeto.move()
        if objeto.qtdColisoes > 8:
            objeto.qtdColisoes = 0
            objeto.v1 *= 1.10
